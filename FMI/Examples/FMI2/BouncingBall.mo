...
block BouncingBall
  ...
  FMI2RealOutput h "Position of the ball" annotation(Placement(transformation(extent={ { 100, -60},  { 120, -40}}),   iconTransformation(extent={ { 100, -60},  { 120, -40}})));
  FMI2RealOutput v "Velocity of the ball" annotation(Placement(transformation(extent={ { 100, 40},  { 120, 60}}),   iconTransformation(extent={ { 100, 40},  { 120, 60}})));

initial algorithm

  FMI.Internal.loadFMU(
    instance=instance,
    unzipdir=Modelica.Utilities.Files.loadResource("modelica://FMI/Resources/FMUs/71b1bae"),
    fmiVersion=2,
...
algorithm
  ...
  when sample(startTime, communicationStepSize) then
    ...
    h := FMI2GetReal(instance, valueReference=1);
    v := FMI2GetReal(instance, valueReference=3);
  ...
  annotation (
    ...
    Documentation(info="<html><p>For more information open the FMU's <a href=\"modelica://FMI/Resources/FMUs/71b1bae/documentation/index.html\">original documentation</a>.</p></html>"));
end BouncingBall;
